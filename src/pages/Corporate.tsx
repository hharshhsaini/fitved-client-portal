...
import {
  ...
  Dumbbell, HeartHandshake, Star, Sparkles, ArrowRight, Phone, Mail,
...
function InquiryForm() {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    const { error } = await supabase.from("leads").insert({
      ...
      phone: form.phone,
      interest: `B2B – ${form.event_type}`,
      source: "corporate_form",
      // Extra context packed into a note field (gracefully ignored if column doesn't exist)
    });
    setBusy(false);
    ...
    <section id="inquiry" className="py-20 md:py-28 bg-fv-neutral">
      <div className="mx-auto max-w-2xl px-4">
        ...
        <div className="mt-8 flex flex-col sm:flex-row items-center justify-center gap-6 text-sm text-fv-text/50">
          <a href="tel:+919999999999" className="flex items-center gap-2 hover:text-fv-navy transition-colors">
            <Phone size={14} /> +91 99999 99999
          </a>
          <a href="mailto:b2b@fitved.in" className="flex items-center gap-2 hover:text-fv-navy transition-colors">
            <Mail size={14} /> b2b@fitved.in
          </a>
        </div>
      </div>
    </section>
    ...
    <footer className="bg-fv-navy text-white/60 py-10">
      <div className="mx-auto max-w-6xl px-4 flex flex-col md:flex-row items-center justify-between gap-4 text-sm">
        ...
        <div className="flex items-center gap-6">
          ...
          <a href="https://instagram.com/fitved.in" target="_blank" rel="noreferrer" className="hover:text-white transition-colors">Instagram</a>
          <a href="mailto:b2b@fitved.in" className="hover:text-white transition-colors">b2b@fitved.in</a>
        </div>
